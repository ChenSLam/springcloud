...
/*
@HystrixCommand 注解的作用是标记当前控制器方法启用Hystrix的熔断机制,当远程服务出现异常或超时就会自动进行熔断
fallbackMethod 用于指定一个本地方法的名称,当熔断以后就会调用这个方法来替代服务提供者的响应信息
* */
    ...
    public String test(){
        ...
        * Spring Cloud会根据这个服务名到注册中心获取服务名所对应的服务所在IP+端口号然后利用RestTemplate访问服务*/
        ResponseEntity<String> result = restTemplate.getForEntity("http://04-EUREKA-CLIENT-RIBBON-PROVIDER/test", String.class);
        ...
        return "使用了Ribbon负载均衡的服务消费者"+body;
    }
    public String error(){
        return "服务被熔断了";
    }
